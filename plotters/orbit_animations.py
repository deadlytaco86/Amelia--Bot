...
def animate(e, a, mass, c_dir):
    ...
    frame_path = f'{c_dir}/bot_data/orbit_plots/'
    save_path = f'{c_dir}/bot_data/graph_data/'
    plot_size = (6,6)
    orbit_points = 400
    G = 6.6743e-11
    AU = 149597870700
    ...
    for i in range(timesteps):
        ...
        plt.savefig(frame_path + graph_name)
    ...
    plt.close()
    #gif_start = time.time() ########################################
    frames = [Image.open(frame_path + filename) for filename in filenames[1:]]
    frame_one = Image.open(frame_path + filenames[0])
    frame_one.save(f'{save_path}Orbit Animation.gif', format="GIF", append_images=frames, save_all=True, duration=1000/gif_fps, loop=0)
    #print(f'gif finished in {time.time() - gif_start}') ########################################
    #print(f'total time is {time.time() - t_start_loading}') ########################################
    for i in range(timesteps):
        os.remove(f'{frame_path}at angle {i}.png')
